...
const publicState = room => ({
  ...
  timer: Math.max(0, Math.ceil((room.endsAt-Date.now())/1000)),
  qa: room.qa,
  scores: Object.fromEntries(room.players.map(p => [p.id,p.score])),
  locations,
  askedIds: [...room.askedIds],
  lastQuestionerId: room.lastQuestionerId
});
...
function endRound(room, title, detail, winner){
  ...
  if(winner){
    const player = room.players.find(p => p.id === winner);
...
function startRound(room){
  ...
  room.qa=[];
  room.pending=null;
  room.askedIds=new Set();
  room.lastQuestionerId=null;
  room.endsAt=Date.now()+600000;
  if(room.timerId) clearTimeout(room.timerId);
  room.timerId=setTimeout(()=>endRound(room,'⏰ Hết thời gian','Đã hết 10 phút. Spy được công bố.',room.spyId),600000);
  for(const p of room.players){
    io.to(p.id).emit('role',{spy:p.id===room.spyId,location:p.id===room.spyId?null:room.location});
  }
  emitRoom(room);
}
...
io.on('connection', socket => {
  socket.on('create_room', ({name})=>{
    const code=makeCode();
    const p={id:socket.id,name:String(name||'Người chơi').trim().slice(0,20)||'Người chơi',score:0};
    const room={code,hostId:p.id,players:[p],phase:'lobby',round:0,turnIndex:0,qa:[],pending:null,askedIds:new Set(),lastQuestionerId:null};
    rooms.set(code,room);
    socket.join(code);
    socket.roomCode=code;
    socket.emit('joined',{room:code,selfId:p.id});
    emitRoom(room);
  });

  socket.on('join_room', ({code,name})=>{
    const room=rooms.get(String(code||'').toUpperCase());
    if(!room) return socket.emit('error_msg','Không tìm thấy phòng.');
    if(room.phase!=='lobby') return socket.emit('error_msg','Ván đã bắt đầu.');
    if(room.players.length>=12) return socket.emit('error_msg','Phòng đã đủ 12 người.');
    const p={id:socket.id,name:String(name||'Người chơi').trim().slice(0,20)||'Người chơi',score:0};
    room.players.push(p);
    socket.join(room.code);
    socket.roomCode=room.code;
    socket.emit('joined',{room:room.code,selfId:p.id});
    emitRoom(room);
  });
  ...
  socket.on('send_question',({targetId,question})=>{
    ...
    if(!q) return;
    room.pending={fromId:socket.id,toId:target.id,q};
    io.to(target.id).emit('question_received',{fromId:socket.id,fromName:room.players.find(p=>p.id===socket.id).name,q});
    emitRoom(room);
  });

  socket.on('answer_question',({answer})=>{
    const room=rooms.get(socket.roomCode);
    if(!room||room.phase!=='game'||!room.pending||room.pending.toId!==socket.id) return;
    const from=room.players.find(p=>p.id===room.pending.fromId);
    const to=room.players.find(p=>p.id===socket.id);
    const a=String(answer||'').trim().slice(0,180);
    if(!from||!to||!a) return;

    room.qa.push({from:from.name,to:to.name,q:room.pending.q,a});
  ...
  socket.on('accuse',({targetId})=>{
    ...
    const target=room.players.find(p=>p.id===targetId);
    if(!target) return;
    if(target.id===room.spyId) endRound(room,'👥 Người thường thắng!',`${target.name} chính là Spy.`,socket.id);
    else endRound(room,'🕵️ Spy thắng!',`${target.name} không phải Spy.`,room.spyId);
  });
  ...
  socket.on('spy_guess',({guess})=>{
    ...
    if(g.toLowerCase()===room.location.toLowerCase()) endRound(room,'🕵️ Spy thắng!',`Spy đoán đúng: ${room.location}.`,room.spyId);
    else endRound(room,'👥 Người thường thắng!',`Spy đoán sai. Địa điểm là ${room.location}.`,null);
  });
  ...
  socket.on('disconnect',()=>{
    const code=socket.roomCode, room=rooms.get(code);
    if(!room) return;
    room.players=room.players.filter(p=>p.id!==socket.id);
    room.askedIds.delete(socket.id);
    if(!room.players.length){rooms.delete(code);return;}
    if(room.hostId===socket.id) room.hostId=room.players[0].id;
    if(room.phase==='game'&&room.spyId===socket.id) endRound(room,'👥 Người thường thắng!','Spy đã rời phòng.',null);
    if(room.phase==='game'&&room.turnIndex>=room.players.length) room.turnIndex=0;
    emitRoom(room);
  });
});

const PORT=process.env.PORT||10000;
server.listen(PORT,'0.0.0.0',()=>console.log(`Spyfall listening on ${PORT}`));